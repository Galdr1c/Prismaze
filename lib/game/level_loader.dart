...
class LevelLoader extends Component with HasGameRef<PrismazeGame> {
  ...
  Future<void> loadLevel(int levelId) async {
    ...
    // Load from JSON assets (supports 1-150+ levels)
    try {
      final jsonString = await rootBundle.loadString('assets/levels/level_$levelId.json');
      final data = jsonDecode(jsonString);
      
      // Add to Cache
      _addToCache(levelId, data);
      
      _processLevelData(levelId, data);
      
    } catch (e) {
      print("Error loading level $levelId: $e");
      // Final fallback: Check hardcoded Campaign Levels
      final campaignLevel = CampaignLevels.levels.where((l) => l.levelNumber == levelId).firstOrNull;
      if (campaignLevel != null) {
        print("Loading Campaign Level $levelId: ${campaignLevel.name}");
        _loadFromDef(campaignLevel);
      }
    }
  }
