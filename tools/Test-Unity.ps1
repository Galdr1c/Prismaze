...
$process = Start-Process -FilePath $UnityPath -ArgumentList "-batchmode -nographics -quit -projectPath `"$project`" -executeMethod Prismaze.Unity.Editor.ProjectSetup.Prepare -logFile `"$prepareLog`"" -WindowStyle Hidden -PassThru -Wait
...
$process = Start-Process -FilePath $UnityPath -ArgumentList "-batchmode -nographics -projectPath `"$project`" -runTests -testPlatform EditMode -testResults `"$testResults`" -logFile `"$testLog`"" -WindowStyle Hidden -PassThru -Wait
...
$process = Start-Process -FilePath $UnityPath -ArgumentList "-batchmode -nographics -projectPath `"$project`" -runTests -testPlatform PlayMode -testResults `"$playResults`" -logFile `"$playLog`"" -WindowStyle Hidden -PassThru -Wait
